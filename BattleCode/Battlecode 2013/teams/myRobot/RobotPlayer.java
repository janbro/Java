...
public class RobotPlayer {
			...
			try{
				...
				if(rc.isActive()){
					...
					if(rc.readBroadcast(getChannel())==1){
						if(rc.senseMine(rc.getLocation().add(rc.getLocation().directionTo(rc.senseEnemyHQLocation())))==Team.NEUTRAL){
					...
					else if((rc.getTeamPower()>80&&(rc.senseEncampmentSquare(rc.getLocation())&&mines+hqEncampments<8)||(hqEncampments==8&&(rc.senseEncampmentSquare(rc.getLocation())&&(rc.getLocation().x%2==1&&rc.getLocation().y%2==1))))){
		...
		while(true){
			try{
				...
				if (rc.isActive()) {
					...
					else if((amountOfSoldiers<15||amountOfSoldiers<enemies.length+10)&&nukeCounter<350){//&&rc.getControlBits()>40){
	...
	public static int getChannel(){
		int channel = (int) ((Clock.getRoundNum()*2432)%GameConstants.BROADCAST_MAX_CHANNELS);
		return channel;
	}
}